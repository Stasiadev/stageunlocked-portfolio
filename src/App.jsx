...
function Nav({ onContact }) {
  ...
  return (
    <nav style={{
      ...
      {/* Logo */}
      <div style={{ fontFamily: FD, fontWeight: 800, fontSize: 17, letterSpacing: 2.5,
      ...
      <div style={{ display: 'flex', gap: 36, alignItems: 'center' }}>
        {['About', 'Projects', 'Skills', 'Contact'].map(item => (
          <a key={item} href={`#${item.toLowerCase()}`} className="nav-link"
            style={{ color: C.dimLt, textDecoration: 'none', fontSize: 14,
    ...
    <section style={{
      ...
      <div style={{ maxWidth: 820, animation: 'fadeUp 1s cubic-bezier(0.16,1,0.3,1) forwards', position: 'relative' }}>
        ...
        <div style={{
          ...
          <span style={{ width: 7, height: 7, borderRadius: '50%', background: C.mg,
        ...
        {/* Stats row */}
        <div style={{ display: 'flex', gap: 48, marginTop: 64, paddingTop: 40,
          ...
          {[
            ...
            <div key={s.label}>
              <div style={{ fontFamily: FD, fontSize: 32, fontWeight: 800,
                background: C.grad, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                {s.num}
              </div>
    ...
    <section id="about" style={{ padding: 'clamp(80px,10vw,130px) clamp(24px,5vw,100px)' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
	    ...
	    />
          ...
          {/* Copy */}
          <div>
            <p style={{ fontSize: 11, color: C.mg, letterSpacing: 2.5, marginBottom: 16,
              fontWeight: 700, fontFamily: FB }}>
              ABOUT ME
            </p>
            <h2 style={{ fontFamily: FD, fontSize: 'clamp(32px,4vw,48px)', fontWeight: 700,
	      ...
	      paddingBottom: 8,
            ...
            <p style={{ color: C.dim, lineHeight: 1.85, marginBottom: 18, fontWeight: 300, fontSize: 15 }}>
            ...
            <p style={{ color: C.dim, lineHeight: 1.85, marginBottom: 36, fontWeight: 300, fontSize: 15 }}>
  ...
  const projects = [
    {
      ...
      desc: 'A responsive financial analytics dashboard built in React — featuring portfolio tracking, SVG charts, asset allocation, and transaction history. Dark-theme UI optimized for data density and clarity.',
      tags: ['React', 'JavaScript', 'CSS3', 'SVG Charts', 'Responsive'],
      live: true,
      featured: true,
    ...
    },
    {
      emoji: '🔐',
      title: 'Coming Soon',
      desc: 'A new project is currently in development. Check back shortly for the full reveal.',
      tags: [],
      live: false,
      featured: false,
    ...
    },
    {
      emoji: '🔐',
      title: 'Coming Soon',
      desc: 'Another project is in the pipeline. More details to follow.',
      tags: [],
      live: false,
      featured: false,
      bg: 'rgba(255,255,255,0.02)',
    ...
    <section id="projects" style={{ padding: 'clamp(80px,10vw,130px) clamp(24px,5vw,100px)' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div className="reveal" style={{ marginBottom: 56 }}>
          <p style={{ fontSize: 11, color: C.mg, letterSpacing: 2.5, marginBottom: 14,
            fontWeight: 700, fontFamily: FB }}>
            SELECTED WORK
          </p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 24 }}>
          {projects.map((p, i) => (
            <div key={i} className={`reveal project-card reveal-d${i}`} style={{
              background: p.bg, borderRadius: 24, padding: 32,
              border: `1px solid ${p.featured ? 'rgba(212,23,138,0.2)' : C.border}`,
              display: 'flex', flexDirection: 'column',
              minHeight: 320,
            }}>
              {/* Icon */}
              <div style={{ fontSize: 44, marginBottom: 22, lineHeight: 1 }}>{p.emoji}</div>

              {/* Featured badge */}
              {p.featured && (
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6,
                  marginBottom: 16, alignSelf: 'flex-start' }}>
                  <span style={{ width: 6, height: 6, borderRadius: '50%', background: C.mg,
                    animation: 'pulse 2s infinite', display: 'inline-block' }} />
                  <span style={{ fontSize: 10, color: C.mg, letterSpacing: 1.5,
                    fontWeight: 700, fontFamily: FB }}>LIVE</span>
                </div>
              )}

              <h3 style={{ fontFamily: FD, fontSize: p.live ? 24 : 20, fontWeight: 700,
                marginBottom: 12, color: p.live ? C.white : C.dim }}>
                {p.title}
              </h3>

              <p style={{ color: C.dim, fontSize: 14, lineHeight: 1.75, flex: 1,
              ...
              {p.live ? (
                ...
                <span style={{ fontSize: 11, color: C.dim, letterSpacing: 1.5,
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 40 }}>
          {cats.map((cat, i) => (
            <div key={i} className={`reveal reveal-d${i % 3}`}>
              <p style={{ fontSize: 11, color: C.mg, letterSpacing: 2, marginBottom: 18,
                fontWeight: 700, fontFamily: FB }}>
    ...
    <section id="contact" style={{ padding: 'clamp(80px,10vw,130px) clamp(24px,5vw,100px)' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <div className="reveal" style={{
          ...
          <h2 style={{ fontFamily: FD, fontWeight: 800,
          ...
          <p style={{ color: C.dim, fontSize: 15, fontWeight: 300, maxWidth: 440,
...
function Footer() {
  return (
    <footer style={{
      ...
      <span style={{ fontFamily: FD, fontWeight: 800, fontSize: 15, letterSpacing: 2,
        background: C.grad, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
        STAGE UNLOCKED
      </span>
